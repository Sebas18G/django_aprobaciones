...
function aprobarSolicitud(solicitudId, comentario = '') {
    ...
    if (!comentario) {
        comentario = prompt('Comentario adicional (opcional):') || '';
    }

    fetch(`/aprobaciones/solicitud/${solicitudId}/aprobar/`, {
        method: 'POST',
        headers: {
    ...
    .catch(err => {
        ...
        showToast(`Error al aprobar la solicitud: ${err}`, 'danger');
    });
}

function rechazarSolicitud(solicitudId, comentario = '') {
    if (!comentario) {
        comentario = prompt('¿Por qué rechazas esta solicitud? (recomendado):');
        if (comentario === null) return; // Usuario canceló
    }

    if (!confirm('¿Estás seguro de que deseas rechazar esta solicitud?')) return;

    fetch(`/aprobaciones/solicitud/${solicitudId}/rechazar/`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRFToken': getCSRFToken()
    ...
    .catch(err => {
        console.error('Error en la petición:', err);
        showToast(`Error al rechazar la solicitud: ${err}`, 'danger');
    });
}